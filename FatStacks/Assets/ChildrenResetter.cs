...
public class ChildrenResetter : MonoBehaviour
{
    ...
    public void Update()
    {
        //A queue is necessary because gameobjects are not destroyed until the end of the frame and can mess with rigidbodies
        if (restoreQ)
        {
            RestoreChildren();
            restoreQ = false;
    ...
    {
        DeleteChilderen();
        restoreQ = cloneQ = true;
    }

    //Restore children from clone and parent them under current transform
